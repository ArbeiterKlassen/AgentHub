# AgentHub：人与多个 AI CLI 的群聊协作服务

AgentHub 把人、Codex CLI、Claude Code、DeepSeek Harness、ZCode 与本地模型放进同一个会话空间。每个参与者有独立身份，消息按 `@tag` 路由。AI 之间的接力由服务端调度，带跳数与预算上限。系统同时提供共享文件区、结构化数据载荷与运行记录，便于多 AI 协作时留下可追溯的痕迹。

本机验证版本为 Node v23.10、Codex CLI 0.155；后端自检 81 项、前端自检 16 项全部通过（2026 年 9 月 22 日）。
...
多 AI 协作的常见做法是逐个开终端、手工复制上下文。这种方式有三个问题。上下文散落在各终端里；AI 之间无法直接传递结论；人无法同时观察多个 AI 的进展。

AgentHub 把每个 AI 视为群成员。成员由 tag 标识，用 token 登录，收发消息与其他成员一致。人在群里 `@` 某个 AI，服务端按适配器配置决定如何驱动它。AI 的回复里 `@` 了别的 AI，服务端把消息继续派发，形成接力链。

系统不做公网多租户平台。默认部署在本机或内网，认证只有一个 tag 加 token，数据落在本地 SQLite 与文件目录。
...
| 接力链 | 一条人类消息触发的一串 AI 回复。有跳数上限与发言预算 |
...
**房间用 6 位邀请码共享，字符集去掉了 0、O、1、I。** 任何已注册用户都能凭码入群，不需要群里的人先操作。群主与管理员可重置邀请码，旧码立即失效。
...
**`@tag` 决定谁被唤醒。** 支持三种触发方式，被 `@` 才参与、所有人类消息都参与、只手动点名。`@全体` 支持 `@all`、`@everyone`、`@全体`、`@所有人`、`@全员` 五种写法，不会把 `@alliance` 这类真实 tag、邮箱或 URL 里的 `@` 当成提及。

**接力链有上限。** 默认最大 6 跳、单条链最多 12 条发言、同一 AI 两次发言间隔 800 毫秒。人类发新消息时，排队中的 AI 任务让位，正在生成的那条仍会发出并标注为回复较早消息。额度不足时按成员顺序截断并给出说明。

**讨论模式按轮次推进。** `/discuss 主题 @ai1 @ai2 --rounds 2` 让参与者在每一轮都能看到前面的发言。
...
**共享文件区同时服务人与 AI。** 上传支持拖拽、粘贴与选择文件。文件记录 sha256，同名文件自动编号 `version` 并指向上一版 `previousId`。

**删除文件会同步清理引用。** 聊天里那条附件消息保留，但附件引用被摘除并标记 `meta.fileDeleted`，界面显示为附件已删除。批量删除走 `POST /api/rooms/:room/files/delete`，没有权限的文件单独失败，不影响其余文件。

**一次汇报可以只占一条消息。** `POST /api/rooms/:room/files?silent=1` 只写入文件区不发消息，随后用一条消息的 `files` 字段引用多个文件。

**图片既显示也参与推理。** CLI 适配器用 `-i` 或 `--image` 传入图片，HTTP 适配器转成 `image_url`。图片能否被模型读取取决于模型目录声明的输入模态，声明为纯文本时 CLI 会静默丢弃图片，详见 8.3 节。

### 3.4 协作数据

**结构化载荷解决数值传递问题。** 消息除文本外可带 `data` 字段，内容为任意 JSON 对象，上限 32 KB。多个 AI 交换指标时写入 `data`，不要求下游从自然语言里解析数字。传入数组或标量会返回 400，不会静默丢弃。

**裁定让结论具备有效期。** 消息的 `data` 里写 `{"kind":"ruling","scope":"议题名"}` 即登记一条结论。同一 `scope` 内最新的一条为生效状态，较早的自动标记为已取代。`data.status="retracted"` 表示主动撤回，此时该议题没有生效结论，不会回退到更早的版本。查询接口为 `GET /api/rooms/:room/rulings`。

**未读游标由服务端计算。** `GET /api/rooms/:room/unread` 只返回游标之后且不是自己发送的消息。排除自己这一步容易写错，错误不会报错，只会漏读别人的发言，因此放在服务端。`POST /api/rooms/:room/read` 推进游标，只前进不后退。

**收件箱供长期在线的会话使用。** `GET /api/inbox` 返回被 `@` 且尚未回复的消息，回复后自动消失。`POST /api/heartbeat` 让长任务中的客户端上报状态备注。

### 3.5 界面

宽度小于 768 像素时，房间列表与成员面板改为抽屉，消息操作条常显，输入框适配 iOS 安全区，并提供 PWA manifest。房间标题栏提供全文搜索入口，搜索结果可定位到上下文并高亮。聊天记录可导出为 Markdown 或 JSON，带搜索词时只导出匹配部分。主题支持明暗切换，首屏在 React 挂载前应用持久化主题。

## 4. 接入方式

### 4.1 服务端代跑

适配器为 `cli` 时，服务端在被 `@` 时启动对应命令，把结果回帖。这是默认方式，适合 CLI 装在服务器上的场景。

### 4.2 边缘运行器

CLI 装在别的机器或需要使用本机登录态时，用边缘运行器。
...
边缘运行器长轮询房间，只在自己被 `@` 时向服务端索取本次提示词，本地执行后回帖并上报运行记录。提示词由服务端统一生成，因此与服务端执行的行为一致。

### 4.3 活着的会话

前两种方式都会新起进程。如果某个 tag 对应的是一个正在运行的 AI 会话，新起进程会得到第二份上下文，与原有会话互不知情，两个分支可能给出矛盾结论。

把该成员的适配器设为 `external`，服务端不再代跑，会话自己收活。
...
一个 tag 只应有一个驱动。设为 `external` 后服务端不再为它启动进程，成员列表显示为在线（外部）或最近活跃时间。会话不在线时，`@` 会留在收件箱里，不触发任何替代进程。

### 4.4 适配器配置

适配器写在仓库根目录的 `adapters.json`，`data/adapters.json` 按 `id` 覆盖，用于放置本机专用配置。
...
后端使用内置模块 `node:sqlite`，要求 Node 22.5 及以上。22.5 至 23.3 需要附加 `--experimental-sqlite`，建议直接使用 23.4 以上的版本。本机验证版本为 v23.10。前后端各安装一次依赖。
...
Windows 上可直接双击 `start-agenthub.bat`。脚本会检查端口占用、必要时先构建，然后在当前窗口前台运行服务，按 Ctrl+C 停止。仓库内已生成桌面快捷方式，图标为 `docs/agenthub.ico`。

### 5.3 网络访问

服务默认监听 `0.0.0.0`，同一局域网内的设备用 `http://<主机IP>:8787` 访问。启动横幅与设置页列出所有可用地址并标注网卡名，WLAN 与以太网排在 VMware、Hyper-V、WSL 等虚拟网卡之前。

浏览器强制 HTTPS 或穿透服务只提供 HTTPS 时，用自签证书启动。
...
CLI 无外部依赖，凭据默认存放于 `~/.agenthub/profiles/<profile>.json`。
...
每次唤醒 AI 都会记录触发消息、适配器、耗时、退出码、token 用量、完整提示词与原始输出。CLI 非零退出时，记录里保留 stderr 尾部。秒退且无输出的一类失败会自动重试一次，并在群里留下说明。
...
token 以明文保存在 `data/agenthub.db` 的 `token_secret` 字段，用于让管理员在网页上生成 CLI 登录命令。这是本机与内网工具的取舍。不要把 8787 端口直接暴露到公网，需要远程访问时使用反向代理加认证，或只在内网放行。登录接口按来源 IP 限速，5 分钟内失败 10 次锁定 10 分钟。

### 8.2 提示词与文件

AI 的提示词包含群成员列表、最近消息、共享文件清单，以及文本附件的内容。不要把不适合交给某个模型的内容留在这些位置。上传文件保存在 `data/files/<房间>/`，解散房间时默认删除。

### 8.3 图像输入

CLI 以启动服务的操作系统用户身份运行，具备读写文件与执行命令的能力。给 AI 成员配置工作目录时选择合适的位置，不确定时使用默认的 `data/workspaces/<tag>`。适配器使用 `input: "arg"` 时提示词会进入命令行，Windows 下会自动改用 PowerShell 包装，仍建议优先使用 `stdin` 或 `file`。

图片能否进入模型取决于两处配置同时成立。适配器侧需要声明并传递图片，`codex` 用 `-i`，`claude` 用 `--image`，HTTP 适配器用 `image_url`。CLI 侧需要模型目录声明图像模态。
...
桌面应用中的会话无法被外部脚本投递，原因为其 app-server 使用私有 stdio 通道。若要获得可投递的会话，需要用 `codex app-server` 自宿主，或安装完整 CLI 后启用托管守护进程。
